...
static const GdkModifierType bmask[4] = {0, GDK_BUTTON1_MASK, 0,
...
/* potential snapping to a named marker time */
  for (i = 0; i < WAVE_NUM_NAMED_MARKERS; i++) {
    if (GLOBALS->named_markers[i] != -1) {
      ...
      if ((GLOBALS->named_markers[i] >= GLOBALS->tims.start) && (GLOBALS->named_markers[i] <= GLOBALS->tims.end) &&
...
static void draw_marker(void) {
  ...
  if (GLOBALS->tims.baseline >= 0) {
    if ((GLOBALS->tims.baseline >= GLOBALS->tims.start) && (GLOBALS->tims.baseline <= GLOBALS->tims.last)
      ...
      if ((xl >= 0) && (xl < GLOBALS->wavewidth)) {
        XXX_gdk_draw_line(cr, GLOBALS->rgb_gc.gc_baseline_wavewindow_c_1, xl, GLOBALS->fontheight - 1, xl,
  ...
  if (GLOBALS->tims.marker >= 0) {
    if ((GLOBALS->tims.marker >= GLOBALS->tims.start) && (GLOBALS->tims.marker <= GLOBALS->tims.last)
      ...
      if ((xl >= 0) && (xl < GLOBALS->wavewidth)) {
        XXX_gdk_draw_line(cr, GLOBALS->rgb_gc.gc_umark_wavewindow_c_1, xl, GLOBALS->fontheight - 1, xl,
                          GLOBALS->waveheight - 1);
  ...
  if (GLOBALS->m1x_wavewindow_c_1 == -1)
    GLOBALS->m1x_wavewindow_c_1 = GLOBALS->m2x_wavewindow_c_1;  /* make both markers same if no ghost marker or v.v. */
  ...
  if (bt) {
    ...
    while (bt) {
      if ((bt->bend < lhs) || (bt->bstart > rhs)) {
        ...
        XXX_gdk_draw_rectangle(GLOBALS->cr_wavepixmap_wavewindow_c_1, GLOBALS->rgb_gc.gc_xfill_wavewindow_c_1, TRUE,
...
#ifdef WAVE_ALLOW_GTK3_GESTURE_EVENT
    if ((gesture_in_zoom) || (!GLOBALS->swipe_init_time) || (GLOBALS->wavearea_gesture_swipe_velocity_x == 0.0) ||
        (old_start != GLOBALS->tims.start)) /* cut down on redundant draws when swiping */
#endif
    {
      XXX_gdk_draw_rectangle(GLOBALS->cr_wavepixmap_wavewindow_c_1, GLOBALS->rgb_gc.gc_back_wavewindow_c_1, TRUE, 0, 0,
                             GLOBALS->wavewidth, GLOBALS->waveheight);
      rendertimebar();
    }
#ifdef WAVE_ALLOW_GTK3_GESTURE_EVENT
...
service_vslider(GtkWidget *text, gpointer data) {
  ...
  if (GLOBALS->cr_signalpixmap) {
    ...
    cairo_set_source_rgba(GLOBALS->cr_signalpixmap, GLOBALS->rgb_gc.gc_ltgray.r, GLOBALS->rgb_gc.gc_ltgray.g,
                          GLOBALS->rgb_gc.gc_ltgray.b, GLOBALS->rgb_gc.gc_ltgray.a);
...
void button_press_release_common(void) {
  ...
  if (GLOBALS->signalarea_has_focus) {
    ...
    cairo_set_source_surface(cr, GLOBALS->surface_signalpixmap,
                             -(gint) (gtk_adjustment_get_value(GTK_ADJUSTMENT(GLOBALS->signal_hslider))), 0);
    cairo_paint(cr);
...
static void button_motion_common(gint xin, gint yin, int pressrel, int is_button_2) {
  ...
  if (!is_button_2) {
    ...
    GLOBALS->tims.baseline = ((GLOBALS->tims.baseline < 0) || (is_button_2 < 0)) ? cook_markertime(newcurr, xin, yin)
                                                                                 : -1;
        ...
        if (GLOBALS->wave_scrolling)
          if (GLOBALS->tims.start > GLOBALS->tims.first) {
            ...
            gtk_adjustment_set_value(GTK_ADJUSTMENT(GLOBALS->wave_hslider),
                                     GLOBALS->tims.marker = time_trunc(GLOBALS->tims.timecache = GLOBALS->tims.start));
...
static void alternate_y_scroll(int delta) {
  ...
  if (target > GLOBALS->traces.visible - num_traces_displayable)
    target = GLOBALS->traces.visible - num_traces_displayable;
...
alt_move_left(gboolean fine_scroll) {
  ...
  ntinc = (TimeType) (((gdouble) GLOBALS->wavewidth) *
                      GLOBALS->nspx);  /* really don't need this var but the speed of ui code is human dependent.. */
...
void alt_zoom_out(GtkWidget *text, gpointer data) {
  ...
  g_signal_emit_by_name(XXX_GTK_OBJECT (GTK_ADJUSTMENT(GLOBALS->wave_hslider)),
...
wavearea_drag_update_event(GtkGestureDrag *gesture,
  ...
  ev.state = (gtk_gesture_single_get_current_button(GTK_GESTURE_SINGLE(gesture)) == 3) ? GDK_BUTTON3_MASK
                                                                                       : GDK_BUTTON1_MASK;
#else
...
gint wavearea_configure_event(GtkWidget *widget, GdkEventConfigure *event) {
  ...
  if (GLOBALS->cr_wavepixmap_wavewindow_c_1) {
    if ((GLOBALS->wavewidth != allocation.width) || (GLOBALS->waveheight != allocation.height)) {
      ...
      GLOBALS->surface_wavepixmap_wavewindow_c_1 = cairo_image_surface_create(CAIRO_FORMAT_RGB24, GLOBALS->wavewidth,
                                                                              allocation.height);
  ...
  if (GLOBALS->swipe_init_time) {
    ...
    if (GLOBALS->wavearea_gesture_swipe_velocity_x < -1.0) {
      ...
      if (!GLOBALS->wavearea_drag_active) {
        TimeType dest = GLOBALS->swipe_init_start - (GLOBALS->wavearea_gesture_swipe_velocity_x * GLOBALS->nspx *
                                                     WAVE_GTK3_SWIPE_VEL_VS_DIST_FACTOR) * (1.0 - decaying);
        ...
        if (ntinc) {
          ...
          if (dest <= (GLOBALS->tims.last - pageinc)) gtk_adjustment_set_value(hadj, GLOBALS->tims.timecache = dest);
            ...
            gtk_adjustment_set_value(hadj, GLOBALS->tims.timecache = GLOBALS->tims.last - pageinc);
...
gtk_container_set_resize_mode(GTK_CONTAINER(table), GTK_RESIZE_IMMEDIATE);
  ...
  g_signal_connect(XXX_GTK_OBJECT(GLOBALS->wavearea), "configure_event", G_CALLBACK(wavearea_configure_event_local),
                   NULL);
#if GTK_CHECK_VERSION(3, 0, 0)
...
/* so far is mutually exclusive with existing motion/button action below */
    ...
    gtkwave_signal_connect(XXX_GTK_OBJECT(GLOBALS->wavearea_gesture_swipe), "swipe", G_CALLBACK(wavearea_swipe_event),
                           GLOBALS);
    gtkwave_signal_connect(XXX_GTK_OBJECT(GLOBALS->wavearea_gesture_swipe), "update",
                           G_CALLBACK(wavearea_swipe_update_event), GLOBALS);
    gtk_widget_add_tick_callback(GTK_WIDGET(GLOBALS->wavearea), wavearea_swipe_tick, NULL, NULL);
  } else
#endif
  {
    gtkwave_signal_connect(XXX_GTK_OBJECT(GLOBALS->wavearea), "motion_notify_event", G_CALLBACK(motion_notify_event),
                           NULL);
    gtkwave_signal_connect(XXX_GTK_OBJECT(GLOBALS->wavearea), "button_press_event", G_CALLBACK(button_press_event),
                           NULL);
    gtkwave_signal_connect(XXX_GTK_OBJECT(GLOBALS->wavearea), "button_release_event", G_CALLBACK(button_release_event),
                           NULL);
  }
  ...
  XXX_gtk_table_attach(XXX_GTK_TABLE (table), GLOBALS->wavearea, 0, 9, 0, 9, GTK_FILL | GTK_EXPAND,
...
void RenderSigs(int trtarget, int update_waves) {
  ...
  if (!GLOBALS->use_dark) {
    cairo_set_source_rgba(GLOBALS->cr_signalpixmap, GLOBALS->rgb_gc.gc_mdgray.r, GLOBALS->rgb_gc.gc_mdgray.g,
                          GLOBALS->rgb_gc.gc_mdgray.b, GLOBALS->rgb_gc.gc_mdgray.a);
    cairo_rectangle(GLOBALS->cr_signalpixmap, 0, -1, GLOBALS->signal_fill_width, GLOBALS->fontheight);
    cairo_fill(GLOBALS->cr_signalpixmap);
  }

  cairo_set_source_rgba(GLOBALS->cr_signalpixmap, GLOBALS->rgb_gc_white.r, GLOBALS->rgb_gc_white.g,
                        GLOBALS->rgb_gc_white.b, GLOBALS->rgb_gc_white.a);
  cairo_move_to(GLOBALS->cr_signalpixmap, WAVE_CAIRO_050_OFFSET, GLOBALS->fontheight - 1 + WAVE_CAIRO_050_OFFSET);
  cairo_line_to(GLOBALS->cr_signalpixmap, GLOBALS->signal_fill_width - 1 + WAVE_CAIRO_050_OFFSET,
                GLOBALS->fontheight - 1 + WAVE_CAIRO_050_OFFSET);
  ...
  if (!GLOBALS->use_dark) {
    XXX_font_engine_draw_string(GLOBALS->cr_signalpixmap, GLOBALS->signalfont, &(GLOBALS->rgb_gc_black),
                                3 + xsrc + WAVE_CAIRO_050_OFFSET, GLOBALS->fontheight - 4 + WAVE_CAIRO_050_OFFSET,
                                "Time");
  }
  ...
  if (GLOBALS->signalarea_has_focus) {
    cairo_set_source_rgba(GLOBALS->cr_signalpixmap, GLOBALS->rgb_gc_black.r, GLOBALS->rgb_gc_black.g,
                          GLOBALS->rgb_gc_black.b, GLOBALS->rgb_gc_black.a);
...
int RenderSig(Trptr t, int i, int dobackground) {
  ...
  if (IsSelected(t)) {
    ...
    if (HasWave(t)) { text_color = GLOBALS->rgb_gc_black; }
    else { text_color = (IsGroupBegin(t) || IsGroupEnd(t)) ? clr_group : clr_comment; }
  }
            ...
            if (!t->n.nd->extvals) {
              ...
              if (t->n.nd->vartype == ND_VCD_EVENT) {
                h_val = (h_ptr->time >= GLOBALS->tims.first) &&
                        ((GLOBALS->tims.marker - GLOBALS->shift_timebase) == h_ptr->time) ? AN_1
...
void MaxSignalLength_2(char dirty_kick) {
  if (!GLOBALS->in_button_press_wavewindow_c_1) {
    if (!GLOBALS->do_resize_signals) {
      ...
      if ((oldusize != rs) || (dirty_kick)) { /* keep signalwindow from expanding arbitrarily large */
        int wx, wy;
...
static void renderhash(int x, TimeType tim) {
  ...
  cairo_set_source_rgba(GLOBALS->cr_wavepixmap_wavewindow_c_1, GLOBALS->rgb_gc.gc_grid_wavewindow_c_1.r,
                        GLOBALS->rgb_gc.gc_grid_wavewindow_c_1.g, GLOBALS->rgb_gc.gc_grid_wavewindow_c_1.b,
  ...
  cairo_line_to(GLOBALS->cr_wavepixmap_wavewindow_c_1, x + WAVE_CAIRO_050_OFFSET,
                (((!timearray_encountered) && (GLOBALS->display_grid) && (GLOBALS->enable_vert_grid))
                 ? GLOBALS->waveheight : fhminus2) + WAVE_CAIRO_050_OFFSET);
  ...
  cairo_move_to(GLOBALS->cr_wavepixmap_wavewindow_c_1, x + WAVE_CAIRO_050_OFFSET,
                GLOBALS->wavecrosspiece + WAVE_CAIRO_050_OFFSET);
  cairo_line_to(GLOBALS->cr_wavepixmap_wavewindow_c_1, rhs + WAVE_CAIRO_050_OFFSET,
                GLOBALS->wavecrosspiece + WAVE_CAIRO_050_OFFSET);
  cairo_stroke(GLOBALS->cr_wavepixmap_wavewindow_c_1);
...
/***********/
  ...
  for (;;) {
    ...
    if ((x - lenhalf >= lastx) || (GLOBALS->pixelsperframe >= 200)) {
      XXX_font_engine_draw_string(GLOBALS->cr_wavepixmap_wavewindow_c_1, GLOBALS->wavefont,
                                  &GLOBALS->rgb_gc.gc_time_wavewindow_c_1, x - lenhalf + WAVE_CAIRO_050_OFFSET,
                                  GLOBALS->wavefont->ascent - 1 + WAVE_CAIRO_050_OFFSET, timebuff);
      lastx = x + lenhalf;
    }
...
static void rendertimebar(void) {
  XXX_gdk_draw_rectangle(GLOBALS->cr_wavepixmap_wavewindow_c_1, GLOBALS->rgb_gc.gc_timeb_wavewindow_c_1, TRUE, 0, -1,
...
static void draw_hptr_trace(Trptr t, hptr h, int which, int dodraw, int kill_grid) {
  ...
  if ((GLOBALS->highlight_wavewindow) && (t) && (t->flags & TR_HIGHLIGHT) && (!GLOBALS->black_and_white) &&
    ...
    XXX_gdk_draw_line(GLOBALS->cr_wavepixmap_wavewindow_c_1, GLOBALS->rgb_gc.gc_grid_wavewindow_c_1,
                      (GLOBALS->tims.start < GLOBALS->tims.first) ? (GLOBALS->tims.first - GLOBALS->tims.start) *
                                                                    GLOBALS->pxns : 0, liney,
                      (GLOBALS->tims.last <= GLOBALS->tims.end) ? (GLOBALS->tims.last - GLOBALS->tims.start) *
    ...
    if (h->v.h_val != AN_Z) {
      ...
      switch (h->v.h_val) {
        ...
        case AN_DASH:
          ...
          break;
        default:
          c = (h->v.h_val == AN_X) ? GLOBALS->rgb_gc.gc_x_wavewindow_c_1 : GLOBALS->rgb_gc.gc_trans_wavewindow_c_1;
      }
      ...
      if (_x0 != _x1) {
        if (is_event) {
          if (h->time >= GLOBALS->tims.first) {
            XXX_gdk_draw_line(GLOBALS->cr_wavepixmap_wavewindow_c_1, GLOBALS->rgb_gc.gc_w_wavewindow_c_1, _x0, _y0, _x0,
                              _y1);
            XXX_gdk_draw_line(GLOBALS->cr_wavepixmap_wavewindow_c_1, GLOBALS->rgb_gc.gc_w_wavewindow_c_1, _x0, _y1,
                              _x0 + 2, _y1 + 2);
            XXX_gdk_draw_line(GLOBALS->cr_wavepixmap_wavewindow_c_1, GLOBALS->rgb_gc.gc_w_wavewindow_c_1, _x0, _y1,
                              _x0 - 2, _y1 + 2);
          }
        ...
        switch (h2val) {
          ...
          case AN_DASH:
            ...
            break;
          default:
            c = (hval == AN_X) ? GLOBALS->rgb_gc.gc_x_wavewindow_c_1 : GLOBALS->rgb_gc.gc_trans_wavewindow_c_1;
        }
        ...
        switch (hval) {
          ...
          case AN_L:  /* L */
            if (GLOBALS->fill_waveform && invert) {
              ...
              XXX_gdk_draw_rectangle(GLOBALS->cr_wavepixmap_wavewindow_c_1, gcxf, TRUE, _x0 + 1, _y0, _x1 - _x0,
                                     _y1 - _y0 + 1);
            }
            XXX_gdk_draw_line(GLOBALS->cr_wavepixmap_wavewindow_c_1,
                              (hval == AN_0) ? GLOBALS->rgb_gc.gc_0_wavewindow_c_1
                                             : GLOBALS->rgb_gc.gc_low_wavewindow_c_1, _x0, _y0, _x1, _y0);
            ...
            if (h2tim <= GLOBALS->tims.end)
              switch (h2val) {
                ...
                case AN_Z:
                  XXX_gdk_draw_line(GLOBALS->cr_wavepixmap_wavewindow_c_1, c, _x1, _y0, _x1, yu);
                  break;
                default:
                  XXX_gdk_draw_line(GLOBALS->cr_wavepixmap_wavewindow_c_1, c, _x1, _y0, _x1, _y1);
                  break;
              }
            break;
          ...
          case AN_DASH: /* - */
            ...
            if (invert) {
              XXX_gdk_draw_rectangle(GLOBALS->cr_wavepixmap_wavewindow_c_1, gcx, TRUE, _x0 + 1, _y0, _x1 - _x0,
                                     _y1 - _y0 + 1);
            } else {
              XXX_gdk_draw_rectangle(GLOBALS->cr_wavepixmap_wavewindow_c_1, gcxf, TRUE, _x0 + 1, _y1, _x1 - _x0,
                                     _y0 - _y1 + 1);
            }
            ...
            if (identifier_str[0]) {
              ...
              if ((width = _x1 - _x0_new) > GLOBALS->vector_padding) {
                if ((_x1 >= GLOBALS->wavewidth) ||
                    (font_engine_string_measure(GLOBALS->wavefont, identifier_str) + GLOBALS->vector_padding <=
                     width)) {
                  XXX_font_engine_draw_string(GLOBALS->cr_wavepixmap_wavewindow_c_1, GLOBALS->wavefont,
                                              &GLOBALS->rgb_gc.gc_value_wavewindow_c_1, _x0 + 2 + WAVE_CAIRO_050_OFFSET,
                                              ytext + WAVE_CAIRO_050_OFFSET, identifier_str);
                }
              }
            }
          ...
          case AN_Z: /* Z */
            XXX_gdk_draw_line(GLOBALS->cr_wavepixmap_wavewindow_c_1, GLOBALS->rgb_gc.gc_mid_wavewindow_c_1, _x0, yu,
                              _x1, yu);
            if (h2tim <= GLOBALS->tims.end)
              switch (h2val) {
                case AN_0:
                case AN_L:
                  XXX_gdk_draw_line(GLOBALS->cr_wavepixmap_wavewindow_c_1, c, _x1, yu, _x1, _y0);
                  break;
                case AN_1:
                case AN_H:
                  XXX_gdk_draw_line(GLOBALS->cr_wavepixmap_wavewindow_c_1, c, _x1, yu, _x1, _y1);
                  break;
                default:
                  XXX_gdk_draw_line(GLOBALS->cr_wavepixmap_wavewindow_c_1, c, _x1, _y0, _x1, _y1);
                  break;
          ...
          case AN_H: /* 1 */
            ...
            XXX_gdk_draw_line(GLOBALS->cr_wavepixmap_wavewindow_c_1,
                              (hval == AN_1) ? GLOBALS->rgb_gc.gc_1_wavewindow_c_1
                                             : GLOBALS->rgb_gc.gc_high_wavewindow_c_1, _x0, _y1, _x1, _y1);
            if (h2tim <= GLOBALS->tims.end)
              switch (h2val) {
                case AN_1:
                case AN_H:
                ...
                case AN_0:
                case AN_L:
                  XXX_gdk_draw_line(GLOBALS->cr_wavepixmap_wavewindow_c_1, c, _x1, _y1, _x1, _y0);
                  break;
                case AN_Z:
                  XXX_gdk_draw_line(GLOBALS->cr_wavepixmap_wavewindow_c_1, c, _x1, _y1, _x1, yu);
                  break;
                default:
                  XXX_gdk_draw_line(GLOBALS->cr_wavepixmap_wavewindow_c_1, c, _x1, _y0, _x1, _y1);
                  break;
        ...
        if (!is_event) {
          XXX_gdk_draw_line(GLOBALS->cr_wavepixmap_wavewindow_c_1, GLOBALS->rgb_gc.gc_trans_wavewindow_c_1, _x1, _y0,
                            _x1, _y1);
        } else {
          XXX_gdk_draw_line(GLOBALS->cr_wavepixmap_wavewindow_c_1, GLOBALS->rgb_gc.gc_w_wavewindow_c_1, _x1, _y0, _x1,
                            _y1);
          XXX_gdk_draw_line(GLOBALS->cr_wavepixmap_wavewindow_c_1, GLOBALS->rgb_gc.gc_w_wavewindow_c_1, _x0, _y1,
                            _x0 + 2, _y1 + 2);
          XXX_gdk_draw_line(GLOBALS->cr_wavepixmap_wavewindow_c_1, GLOBALS->rgb_gc.gc_w_wavewindow_c_1, _x0, _y1,
...
/* type = (!(h->flags&(HIST_REAL|HIST_STRING))) ? vtype(t,h->v.h_vector) : AN_COUNT; */
    ...
    if (_x0 != _x1) {
      if (type == AN_Z) {
        if (GLOBALS->use_roundcaps) {
          XXX_gdk_draw_line(GLOBALS->cr_wavepixmap_wavewindow_c_1, GLOBALS->rgb_gc.gc_mid_wavewindow_c_1, _x0 + 1, yu,
                            _x1 - 1, yu);
        } else {
          XXX_gdk_draw_line(GLOBALS->cr_wavepixmap_wavewindow_c_1, GLOBALS->rgb_gc.gc_mid_wavewindow_c_1, _x0, yu, _x1,
                            yu);
        }
      } else {
        ...
        if (GLOBALS->use_roundcaps) {
          ...
          if (type == AN_1)
            XXX_gdk_draw_line(GLOBALS->cr_wavepixmap_wavewindow_c_1, c, _x0 + 2, _y1 + 1, _x1 - 2, _y1 + 1);
        } else {
        ...
        if ((width = _x1 - _x0) > GLOBALS->vector_padding) {
              ...
              if (cb.a != 0.0) {
                ...
                if (memcmp(&GLOBALS->rgb_gc.gc_back_wavewindow_c_1, &GLOBALS->rgb_gc_white, sizeof(wave_rgb_t))) {
                  if (!GLOBALS->black_and_white)
                    XXX_gdk_draw_rectangle(GLOBALS->cr_wavepixmap_wavewindow_c_1, cb, TRUE, _x0 + 1, _y1 + 1, width - 1,
                                           (_y0 - 1) - (_y1 + 1) + 1);
                }
          ...
          if ((_x1 >= GLOBALS->wavewidth) ||
              (font_engine_string_measure(GLOBALS->wavefont, ascii2) + GLOBALS->vector_padding <= width)) {
            XXX_font_engine_draw_string(GLOBALS->cr_wavepixmap_wavewindow_c_1, GLOBALS->wavefont,
                                        &GLOBALS->rgb_gc.gc_value_wavewindow_c_1, _x0 + 2 + WAVE_CAIRO_050_OFFSET,
                                        ytext + WAVE_CAIRO_050_OFFSET, ascii2);
          } else {
            char *mod;
            ...
            if (srch_for_color) {
              *srch_for_color = 0;
              ...
              if (cb.a != 0.0) {
                ...
                if (memcmp(&GLOBALS->rgb_gc.gc_back_wavewindow_c_1, &GLOBALS->rgb_gc_white, sizeof(wave_rgb_t))) {
                  if (!GLOBALS->black_and_white)
                    XXX_gdk_draw_rectangle(GLOBALS->cr_wavepixmap_wavewindow_c_1, cb, TRUE, _x0, _y1 + 1, width,
                                           (_y0 - 1) - (_y1 + 1) + 1);
                }
              } else {
                *srch_for_color = '?'; /* replace name as color is a miss */
      ...
      if (type == AN_Z) {
        ...
        if ((width = _x1 - _x0) > GLOBALS->vector_padding) {
          ...
          if ((_x1 >= GLOBALS->wavewidth) ||
              (font_engine_string_measure(GLOBALS->wavefont, ascii2) + GLOBALS->vector_padding <= width)) {
            XXX_font_engine_draw_string(GLOBALS->cr_wavepixmap_wavewindow_c_1, GLOBALS->wavefont,
                                        &GLOBALS->rgb_gc.gc_value_wavewindow_c_1, _x0 + 2, ytext, ascii2);
          } else {
            char *mod;
